...
export async function addMovieUser(uuid, mode, button, showButtons = [], hideButtons = [], div) {
    ...
    await handleAction(() => addUserMovie(userId, uuid), mode, button, "add", showButtons, hideButtons, div);
}

export async function deleteMovieUser(uuid, mode, button, showButtons = [], hideButtons = [], div) {
    const userId = await getUserId();
    await handleAction(() => removeUserMovie(userId, uuid), mode, button, "delete", showButtons, hideButtons, div);
}

export async function toseeMovieUser(uuid, mode, button, showButtons = [], hideButtons = [], div) {
    const userId = await getUserId();
    await handleAction(() => updateSeenUserMovie(userId, uuid, false), mode, button, "tosee", showButtons, hideButtons, div)
}

export async function seenMovieUser(uuid, mode, button, showButtons = [], hideButtons = [], div) {
    const userId = await getUserId();
    await handleAction(() => updateSeenUserMovie(userId, uuid, false), mode, button, "seen", showButtons, hideButtons, div)
}